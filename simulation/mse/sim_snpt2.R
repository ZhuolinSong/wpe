...
sparse_snpt <- mclapply(v_n, n_loop <- function(n){
...
save(sparse_snpt, file = "sparse_snpt.RData")


random_snpt <- mclapply(v_n, n_loop <- function(n){
    mclapply(v_del, del_loop <- function(del){
    mclapply(v_C, C_loop <- function(sigx){
    mclapply(v_mu, mu_loop <- function(mu){
    mclapply(1:k, sim_loop <- function(i){
...
save(random_snpt, file = "random_snpt.RData")