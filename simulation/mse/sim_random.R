...
random_design <- mclapply(1:k, FUN = function(i){
    ...
    lapply(seq_along(v_mu), FUN = function(mu){
        ...
        if (idx==3){
            ...
            fit6 = getA1_new_eig(sim$t, sim$y, r=4, newt=grid, mu=mu0f)
        }else if(idx==1){
...
save(random_design, file = "random_design.RData")